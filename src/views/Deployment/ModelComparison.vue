    ...
    <div class="comparison-container">
        <div class="tool-header premium-header">
            <div class="header-content">
                <h3 class="gradient-text"><i class="el-icon-data-analysis"></i> 模型对比</h3>
                <p>对比多个训练任务的指标曲线和参数配置。</p>
            </div>
            <div class="tool-actions">
                <el-button class="action-btn" size="medium" @click="showSelector = true">
                ...
                >
                    <el-button class="action-btn" size="medium" type="success" plain :disabled="!hasQualifiableRuns">
                ...
                <el-button class="action-btn" size="medium" type="primary" plain :loading="exporting" :disabled="!canExport"
        ...
        <div class="baseline-strip glass-panel-sm" v-if="selectedRuns.length > 0">
            <div class="baseline-left">
                ...
                <el-select v-model="baselineCandidateRunId" size="small" placeholder="选择基准模型" class="baseline-select">
                    <el-option v-for="run in selectedRuns" :key="getRunId(run)" :label="getRunLabel(run)"
                        :value="getRunId(run)" />
                </el-select>
                <el-button size="small" @click="applyBaselineSelection"
                    :disabled="!baselineCandidateRunId">设为基准</el-button>
                <el-button size="small" type="primary" plain :loading="baselineSaving" :disabled="!canPersistBaseline"
                    @click="persistBaselineSelection">
                    保存为项目基准
                </el-button>
                <el-button size="small" type="text" :disabled="!canPersistBaseline || !persistedBaselineRunId"
        ...
        <div class="content-wrapper" v-if="selectedRuns.length > 0">
            <div class="tabs-header">
                <div class="tab-item" :class="{ active: activeTab === 'curves' }" @click="activeTab = 'curves'">
                    <i class="el-icon-s-marketing"></i> 训练曲线
                </div>
                <div class="tab-item" :class="{ active: activeTab === 'params' }" @click="activeTab = 'params'">
                    <i class="el-icon-s-grid"></i> {{ metricCompareTabLabel }}
                </div>
                <div class="tab-item" :class="{ active: activeTab === 'training-params' }"
            ...
            <div class="tab-content" v-show="activeTab === 'params'">
                <div class="params-table-wrapper glass-panel-sm" v-loading="loadingParams">
                    <el-table ref="paramsTable" :data="metricRows" row-key="key" border style="width: 100%"
                        height="100%">
                        <el-table-column prop="key" label="指标" width="180" fixed></el-table-column>
                        <el-table-column v-for="run in selectedRuns" :key="getRunId(run)" :label="getRunLabel(run)"
                            min-width="150">
                ...
                <div class="baseline-diff-wrapper glass-panel-sm" v-if="baselineDiffRows.length">
                    ...
                    <el-table :data="baselineDiffRows" border style="width: 100%">
                        ...
                        <el-table-column v-for="run in selectedRuns" :key="`diff-${getRunId(run)}`"
                            :label="getRunLabel(run)" min-width="190">
                            <template slot-scope="scope">
                                <div class="metric-diff-cell">
                                    ...
                                    <div :class="['metric-delta', scope.row.trendByRun[getRunId(run)] || 'neutral']">
                                        {{ CompareIcon(scope.row.trendByRun[getRunId(run)]) }}
                                        {{ formatDelta(scope.row.deltaByRun[getRunId(run)]?.delta_abs) }}
            ...
            <div class="tab-content" v-show="activeTab === 'training-params'">
                <div class="params-table-wrapper glass-panel-sm" v-loading="loadingTrainingParams">
                    <el-table ref="trainingParamsTable" :data="trainingParamRows" row-key="key" border
                        style="width: 100%" height="100%">
                        <el-table-column prop="key" label="训练参数" width="220" fixed></el-table-column>
                        <el-table-column v-for="run in selectedRuns" :key="`param-${getRunId(run)}`"
                            :label="getRunLabel(run)" min-width="170">
                            <template slot-scope="scope">
        ...
        <el-dialog title="选择训练任务" :visible.sync="showSelector" width="900px" top="5vh" append-to-body>
            <div class="selector-body">
                <div class="filter-bar">
                    <el-input v-model="searchQuery" placeholder="搜索任务名称..." prefix-icon="el-icon-search" size="small"
                        clearable @input="fetchRuns"></el-input>
                    <el-select v-model="projectFilter" placeholder="项目" size="small" clearable filterable
                        @change="fetchRuns">
                        <el-option v-for="p in projectList" :key="p.project_id" :label="p.name"
                            :value="p.project_id"></el-option>
                    </el-select>
                ...
                <el-table ref="runTable" :data="runList" height="500" v-loading="loadingList"
                    @selection-change="handleSelectionChange" row-key="run_id">
                ...
                <div class="pagination-bar">
                    <el-pagination background layout="prev, pager, next" :total="totalRuns" :page-size="pageSize"
                        :current-page.sync="currentPage" @current-change="fetchRuns">
                    </el-pagination>
                </div>
            </div>
            <span slot="footer">
                ...
                <el-button type="primary" @click="confirmSelection"
                    :disabled="tempSelection.length < 1 || tempSelection.length > 7">确定</el-button>
            </span>
        </el-dialog>
    </div>
</template>
