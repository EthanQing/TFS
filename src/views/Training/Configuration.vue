      ...
      return String(value);
        ...
        displayName(name){
            ...
            const map = {
              ...
              resume_job_id: '恢复任务 ID',
              resume_from_epoch: '起始轮次',
              config_path: '配置文件路径',
              eval_during_train: '训练中评估',
              eval_interval: '评估间隔(轮)',
              ...
              'additional_params.torch_version': 'Torch 版本',
              'additional_params.paddle_version': 'Paddle 版本',
              'additional_params.ultralytics_version': 'Ultralytics 版本',
              'augmentation.hsv_h': 'HSV 色调增强',
              'augmentation.hsv_s': 'HSV 饱和度增强',
              'augmentation.hsv_v': 'HSV 亮度增强',
              'augmentation.bgr': 'BGR 通道交换概率',
              'augmentation.degrees': '旋转角度',
              'augmentation.translate': '平移比例',
              'augmentation.scale': '缩放比例',
              'augmentation.shear': '剪切角度',
              'augmentation.perspective': '透视变换',
              'augmentation.flipud': '上下翻转概率',
              'augmentation.fliplr': '左右翻转概率',
              'augmentation.mosaic': 'Mosaic 概率',
              'augmentation.mixup': 'MixUp 概率',
              'augmentation.cutmix': 'CutMix 概率',
              'augmentation.close_mosaic': '关闭 Mosaic 的最后轮数',
              'augmentation.copy_paste': 'Copy-Paste 概率',
              'augmentation.copy_paste_mode': 'Copy-Paste 模式',
              'augmentation.auto_augment': 'AutoAugment 策略',
              'augmentation.erasing': '随机擦除概率',
              'loss_weights.box': '边界框损失权重',
              'loss_weights.cls': '分类损失权重',
              'loss_weights.dfl': 'DFL 损失权重'
            };
